...
export const getI18nSettings = () => {
  return {
    dayNamesShort: weeks.map(week => datepicker.weeks[week]),
    dayNames: weeks.map(week => datepicker.weeks[week]),
    monthNamesShort: months.map(month => datepicker.months[week]),
    monthNames: months.map((month, index) => datepicker.months[week]),
    amPm: ['am', 'pm']
  };
};
